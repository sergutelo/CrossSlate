...
static bool weatherPreviousAutoReconnect = true;
...
static void processScanResults() {
  ...
  if (networkCount > 0 && networks[0].saved) {
    ...
    if (getSavedPassword(networks[0].ssid, savedPass, sizeof(savedPass))) {
      ...
      DBG_PRINTLN("[SYNC] Auto-connecting to saved network");
      return;
    }
  }
}
...
static void beginConnect(const char* ssid, const char* pass) {
  ...
  screenDirty = true;
  DBG_PRINTLN("[SYNC] Connecting to network");
}

static void enterSyncingState() {
  resetSyncTracking();
  // Populate the system clock for the dashboard while WiFi is available.
  configTzTime("CET-1CEST,M3.5.0/2,M10.5.0/3", "pool.ntp.org", "time.cloudflare.com");
  startHttpServer();
  ...
  switch (syncState) {
    ...
    case SyncState::SAVE_PROMPT:
      ...
      if (keyCode == HID_KEY_UP || keyCode == HID_KEY_ENTER) {
        ...
        DBG_PRINTLN("[SYNC] Saved credentials");
        enterSyncingState();
    ...
    case SyncState::FORGET_PROMPT:
      ...
      if (keyCode == HID_KEY_UP || keyCode == HID_KEY_ENTER) {
        ...
        DBG_PRINTLN("[SYNC] Forgot credentials");
        beginScan();
      } else if (keyCode == HID_KEY_DOWN || keyCode == HID_KEY_ESCAPE) {
...
void wifiSyncStart() {
  ...
  // ESP32-C3 has one 2.4GHz radio: CrossSlate never keeps BLE input and WiFi
  // active together. Reconnect resumes after wifiSyncStop().
  autoReconnectEnabled = false;
  if (isDeviceScanning()) stopDeviceScan();
  if (isKeyboardConnected()) disconnectCurrentDevice();
  syncActive = true;
...
bool wifiWeatherConnectSaved(uint32_t timeoutMs, char* status, size_t statusSize) {
  ...
  if (syncActive) {
    if (status && statusSize) snprintf(status, statusSize, "Sync Wi-Fi activo");
    return false;
  }
  // ESP32-C3 has one shared radio. This mirrors Sync's radio hand-off without
  // changing any of its states or exposing SSIDs/passwords in logs.
  weatherPreviousAutoReconnect = autoReconnectEnabled;
  autoReconnectEnabled = false;
  if (isDeviceScanning()) stopDeviceScan();
  if (isKeyboardConnected()) disconnectCurrentDevice();

  Preferences weatherPrefs;
  if (!weatherPrefs.begin("wifi_creds", true)) {
    if (status && statusSize) snprintf(status, statusSize, "Wi-Fi no disponible");
    return false;
  }
  int count = weatherPrefs.getInt("wifi_count", 0);
  // The importer merges: adds CrossInk networks missing here (by SSID) and
  // keeps whatever the user saved manually. It opens wifi_creds read/write in
  // its own scope, so re-open read-only afterwards.
  if (crossinkImportWifiCredentials()) {
    if (status && statusSize) snprintf(status, statusSize, "Wi-Fi heredado de CrossInk");
  }
  weatherPrefs.end();
  if (!weatherPrefs.begin("wifi_creds", true)) {
    if (status && statusSize) snprintf(status, statusSize, "Wi-Fi no disponible");
  ...
  count = weatherPrefs.getInt("wifi_count", 0);
  if (count <= 0) {
    weatherPrefs.end();
    if (status && statusSize) snprintf(status, statusSize, "No hay Wi-Fi guardado");
    return false;
  }
  ...
  if (!connected) {
    ...
    if (status && statusSize) snprintf(status, statusSize, "No se pudo conectar Wi-Fi");
    return false;
  }
  // TLS verification needs a real clock. SNTP typically resolves in 1-3 s;
  // the previous 900 ms budget expired before the first NTP reply arrived.
  configTzTime("CET-1CEST,M3.5.0/2,M10.5.0/3", "time.cloudflare.com", "pool.ntp.org");
  const unsigned long timeDeadline = millis() + 8000;
  while (time(nullptr) < 1700000000 && millis() < timeDeadline) delay(50);
  if (time(nullptr) < 1700000000) {
    wifiWeatherDisconnect();
    if (status && statusSize) snprintf(status, statusSize, "Hora TLS no disponible");
    return false;
  }
  return true;
}
...
void wifiWeatherDisconnect() {
  WiFi.disconnect(true);
  WiFi.mode(WIFI_OFF);
  // Restore the user-visible BLE reconnect policy that was active before the
  // short weather request took ownership of the shared 2.4 GHz radio.
  autoReconnectEnabled = weatherPreviousAutoReconnect;
}
