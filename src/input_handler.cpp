...
// --- Cities menu state (defined in ui_renderer.cpp) ---
extern int citySelection;
extern unsigned long citySavedAtMs;
  ...
  switch (currentState) {
    case UIState::DASHBOARD:
      if (event.keyCode == HID_KEY_ESCAPE) {
        dashboardReload();
        screenDirty = true;
      } else if (event.keyCode == HID_KEY_R) {
    ...
    case UIState::MAIN_MENU: {
      ...
      if (event.keyCode == HID_KEY_DOWN) {
        ...
        if (mainMenuSelection == 0) {
          currentState = UIState::DASHBOARD;
          screenDirty = true;
        } else if (mainMenuSelection == 1) {
          currentState = UIState::READING_PAGE;
          screenDirty = true;
          ...
          screenDirty = true;
        } else if (mainMenuSelection == baseCount && otaBootSwitchToCrossInk()) {
          delay(50);
          ESP.restart();
        }
      }
    ...
    case UIState::SETTINGS: {
      const int SETTINGS_COUNT = 7;  // Orientation, Dark Mode, Writing Mode, Font Size, Bluetooth, Paired Keyboards, Cities
      ...
      // Enter or Right: cycle setting forward
        ...
        if (settingsSelection == 0) {
          ...
          // Cursor starts on the city currently in use.
          citySelection = dashboardGetCityIndex();
          citySavedAtMs = 0;
          currentState = UIState::CITY_SELECTION;
        }
        screenDirty = true;
    ...
    case UIState::CITY_SELECTION: {
      // Auto-dismiss the "Ciudad: X" banner after 1.5 s (e-ink keeps the last
      // frame, so an explicit redraw is needed to clear it).
      if (citySavedAtMs != 0 && millis() - citySavedAtMs >= 1500) {
        citySavedAtMs = 0;
        screenDirty = true;
      }
      if (event.keyCode == HID_KEY_DOWN) {
        citySelection = (citySelection + 1) % CITY_COUNT;
        DBG_PRINTF("[INPUT] Cities: Down -> %d (%s)\n", citySelection, CITIES[citySelection].name);
        screenDirty = true;
      } else if (event.keyCode == HID_KEY_UP) {
        citySelection = (citySelection - 1 + CITY_COUNT) % CITY_COUNT;
        DBG_PRINTF("[INPUT] Cities: Up -> %d (%s)\n", citySelection, CITIES[citySelection].name);
        screenDirty = true;
      } else if (event.keyCode == HID_KEY_ENTER) {
        // Confirm: persist to /crossslate/settings.json (atomic) and switch
        // the weather source. The next dashboard refresh (Right button) uses it.
        if (dashboardSetCityIndex(citySelection)) {
          DBG_PRINTF("[INPUT] Cities: guardada %s\n", CITIES[citySelection].name);
          citySavedAtMs = millis();  // shows the "Ciudad: X" banner; auto-clears
        } else {
          DBG_PRINTLN("[INPUT] Cities: fallo al guardar");
        }
        screenDirty = true;
      } else if (event.keyCode == HID_KEY_ESCAPE) {
        // Back without saving: the selection stays whatever was last confirmed.
        citySavedAtMs = 0;
        currentState = UIState::SETTINGS;
        screenDirty = true;
      }
